...
nmap <silent> \crk <Plug>(coc-cursors-word)
nnoremap \crw :CocCommand document.renameCurrentWord
...
noremap <silent> <F2> :AsyncRun! -cwd=<root> findstr /n /s /C:"" "\%CD\%\*.h" "\%CD\%\*.c*" "\%CD\%\*.py" "\%CD\%\*.js" "\%CD\%\*.vim" 
nnoremap <silent> <F4> :AsyncRun -cwd=<root> cmake . 
nnoremap <silent> <F6> :AsyncRun -cwd=<root> -raw make test 
nnoremap <silent> <F8> :AsyncRun -cwd=<root> -mode=4 make run 
nnoremap <silent> <F7> :AsyncRun -cwd=<root> make 
nnoremap <silent> <F5> :call ExecuteFile()
nnoremap <silent> <F9> :AsyncRun gcc -Wall -O2 "$(VIM_FILEPATH)" -o "$(VIM_FILEDIR)/$(VIM_FILENOEXT)" 
...
set errorformat=%f(%l)\ \\=:\ %t%*\\D%n:\ %m,%*[^\"]\"%f\"%*\\D%l:\ %m,%f(%l)\ \\=:\ %m,%*[^\ ]\ %f\ %l:\ %m,%f:%l:%c:%m,%f(%l):%m,%f:%l:%m,%f|%l|\ %m,[%f:%l]\ ->\ %m,[%f:%l]:%m
set expandtab
...
set runtimepath+=~\\.vim\\bundles\\vim-easymotion
set runtimepath+=~\\.vim\\bundles\\vim-diff-enhanced
set runtimepath+=~\\.vim\\bundles\\asyncrun.vim
set runtimepath+=~\\.vim\\bundles\\vim-startify
set runtimepath+=~\\.vim\\bundles\\vim-colorschemes
set runtimepath+=~\\.vim\\bundles\\vim-misc
set runtimepath+=~\\.vim\\bundles\\errormarker.vim
set runtimepath+=~\\.vim\\bundles\\vim-preview
set runtimepath+=~\\.vim\\bundles\\LeaderF
set runtimepath+=~\\.vim\\bundles\\delimitMate
set runtimepath+=~\\.vim\\bundles\\vim-dirvish
set runtimepath+=~\\.vim\\bundles\\wildfire.vim
set runtimepath+=~\\.vim\\bundles\\vim-surround
set runtimepath+=~\\.vim\\bundles\\vim-devicons
set runtimepath+=~\\.vim\\bundles\\vim-gutentags
set runtimepath+=~\\.vim\\bundles\\gutentags_plus
set runtimepath+=~\\.vim\\bundles\\vim-textobj-user
set runtimepath+=~\\.vim\\bundles\\vim-textobj-indent
set runtimepath+=~\\.vim\\bundles\\vim-textobj-syntax
set runtimepath+=~\\.vim\\bundles\\vim-textobj-parameter
set runtimepath+=~\\.vim\\bundles\\vim-textobj-python
set runtimepath+=~\\.vim\\bundles\\vim-textobj-uri
set runtimepath+=~\\.vim\\bundles\\vim-ps1
set runtimepath+=~\\.vim\\bundles\\vim-lua
set runtimepath+=~\\.vim\\bundles\\vim-cpp-enhanced-highlight
set runtimepath+=~\\.vim\\bundles\\python-syntax
set runtimepath+=~\\.vim\\bundles\\ale
set runtimepath+=~\\.vim\\bundles\\echodoc.vim
set runtimepath+=~\\.vim\\bundles\\coc.nvim
set runtimepath+=~\\.vim\\bundles\\spaceline.vim
set runtimepath+=~\\.vim\\bundles\\vim-buffet
set runtimepath+=D:\\soft\\Vim/vimfiles
set runtimepath+=D:\\soft\\Vim\\vim82
set runtimepath+=D:\\soft\\Vim/vimfiles/after
set runtimepath+=~/vimfiles/after
set runtimepath+=~\\vimfiles
set runtimepath+=~\\.vim\\bundles\\vim-textobj-python\\after
set runtimepath+=~\\.vim\\bundles\\vim-cpp-enhanced-highlight\\after
set runtimepath+=~/.vim
set scrolloff=5
set shiftwidth=4
...
setlocal statusline=%#HomeMode#\ %{spaceline#vimode#vim_mode()}%#HomeModeRight#%#emptySeperate1#%#CocBar#\ %{spaceline#status#coc_status()}%=%#VistaNearest#%{spaceline#vista#vista_nearest()}%#LineInfoLeft#%#StatusEncod#\ %{spaceline#file#file_encode()}\ %#StatusFileFormat#%{spaceline#file#file_format()}%#LineFormatRight#%#StatusLineinfo#%{spaceline#file#file_type()}%#EndSeperate#%{spaceline#scrollbar#scrollbar_instance()}
